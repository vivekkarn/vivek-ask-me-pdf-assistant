...
class OpenRouter_Client {
	...
	public function embed( $text ) {
		...
		if ( empty( $api_key ) ) {
			return new \WP_Error( 'missing_api_key', __( 'OpenRouter API key is not configured.', 'ask-me-ai' ) );
		}
		...
		if ( $code < 200 || $code >= 300 ) {
			return new \WP_Error( 'embedding_failed', $this->extract_error_message( $body, __( 'Embedding request failed.', 'ask-me-ai' ) ) );
		}

		$vector = $body['data'][0]['embedding'] ?? null;
		if ( ! is_array( $vector ) ) {
			return new \WP_Error( 'invalid_embedding_response', __( 'Embedding response did not include a vector.', 'ask-me-ai' ) );
		}
	...
	public function chat( $messages, $model, $temperature = 0.2 ) {
		...
		if ( $code < 200 || $code >= 300 ) {
			return new \WP_Error( 'chat_failed', $this->extract_error_message( $body, __( 'Chat completion request failed.', 'ask-me-ai' ) ) );
		}

		$content = $body['choices'][0]['message']['content'] ?? '';
		if ( '' === trim( $content ) ) {
			return new \WP_Error( 'empty_chat_response', __( 'The AI provider returned an empty response.', 'ask-me-ai' ) );
		}
